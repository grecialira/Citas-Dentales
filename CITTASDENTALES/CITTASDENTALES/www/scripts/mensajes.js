$(document).ready(function () {
    ...
    $('#TablaMensaje').on('click', '.editar', function () {
    ...
    $('#TablaMensaje').on('click', '.eliminar', function () {
        var id = $(this).data('id');    // recupera el Id del alumno
    ...
    leerMensajes(); // Lee los alumnos al terminar de cargar la pagina
...
function AgregarMensajes() {
    ...
    var metodo = "AgregarMensaje";       // Metodo por defecto Agregar
    if (alu.Id > 0)
        metodo = "ActualizarMensaje";    // Cuando se edita, el campo oculto es diferente de 0
    // Peticion de ajax
    $.ajax({
        type: 'POST',
...
function leerMensajes() {
    $('#TablaMensaje').html(''); // Limpia la tabla antes de leer datos
    $.ajax({
        type: "POST",
        url: "http://clinicalajoya.somee.com/Citas_Services.asmx/GetAlumnos",
        data: JSON.stringify({ IdUsuario: '1' }),
        dataType: 'json',
        contentType: 'application/json',
        error: function (xhr) {
            $('#TablaMensaje').html('<tr><td colspan="7">Sin datos</td></tr>');
        },
        success: function (data) {
            ...
            for (i = 0; i < alums.length; i++) {    // Recorre el arreglo
                ...
                lineas +=
                    ...
                    '<td>' + alums[i].name + '</td>' +
                    '<td>' + alums[i].email + '</td>' +
                    '<td>' + alums[i].message + '</td>' +
                    '</tr>'
            }

            $('#TablaMensaje').html(lineas);    // Agrega las lineas creadas a la tabla de alumnos
        }
    });
}
